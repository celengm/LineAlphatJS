...
const myBot = ['ue9acf505543f91a03877e4c25602920d'];
...
class LINE extends LineAPI {
    ...
    {
        var [ cmd_1,payload_1 ] = textMessages.split(' ');
        var [ cmd_2,payload_2 ] = textMessages.split(' ');
        var [ cmd_3,payload_3 ] = textMessages.split(':$:');
        cmd_1 = cmd_1.toLowerCase();
        cmd_2 = cmd_2.toLowerCase();
        cmd_3 = cmd_3.toLowerCase();
        ...
        if(txt == 'setmimic') {
            this.mimicSend = 1;
            this.mimicMIDContact = [];
            await this._sendMessages(seq,"send contact to mimic..")
        }
        ...
        if(txt == 'help' && isAdminOrBot(seq.from)) {
            this._sendMessage(seq, 'Si KeCe Help menu.\n\n[Respon] - [untuk cek respon bot]\n[Speed]- [cek speed bot]\n[Point] - [rec sider]\n[Sider] - [cek sider]\n[ourl/curl] - [buka tutup Qr]\n[cancel] - [cancel all mem]\n[tag all] -[tag all member]');
        }
        ...
        const action = ['cancel on','cancel off','kick on','kick off','mimic on','mimic off']
